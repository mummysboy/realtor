...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    // Send rejection SMS
    try {
      const message = `Your viewing request for ${listing?.title || 'the property'} at ${viewingRequest.timeSlot} has been declined. Please contact us for alternative arrangements.`;
      
      const snsParams = {
        TopicArn: SNS_TOPIC_ARN,
        Message: message,
        Subject: 'Viewing Request Declined',
      };

      await sns.publish(snsParams).promise();
    } catch (snsError) {
      console.error('Error sending rejection SMS:', snsError);
    ...
    console.error('Error denying viewing request:', error);
